...
void ofApp::draw() {
	...
	for (auto deg = 0; deg < 360; deg += 36) {
		...
		auto noise = ofNoise(glm::vec3(location * 0.1, ofGetFrameNum() * 0.025));
		auto noise_radius = ofMap(noise, 0, 1, radius * 0.2, radius * 0.8);
		std::cout << deg << ":" << noise_radius << " [" << noise << "] " << location << ", " << radius * 0.2 << "`" << radius * 0.8 << std::endl;
