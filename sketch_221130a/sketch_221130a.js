...
v0 = createVector(d.x, d.y)
  ...
  size = 1
    ...
    // outer lower
    // y = height - h/2 + (abs(width*0.5-x)/width)*300
    y = height - h*0.25 + (abs(width*0.5-x)/width)*100
    // y = height*3/4 - h/2 + (abs(width*0.5-x)/width)*300
    v2 = createVector(x, y)
...
//drawGradientLayer(width/2, height/3)
//drawBtnDarkLayer(width/2, height,0.00005566)
pop()
  }
...
function draw() {
  ...
  //   v2 = createVector(buildings[0].x -width/2,buildings[0].y + height)
  //   ellipse(d.x, d.y,10)
  //   drawArrow(v0, v1, 'red');
